...
async function initializeBackfill(days = 30) {
  ...
  if (state?.backfill_cursor_block != null && state?.backfill_target_block != null) return state;
  const [startBlock, latestBlock] = await Promise.all([
    getBlockAtTimestamp(Date.now() - days * DAY_MS),
    getLatestBlock(),
  ]);
  await upsertState({
    backfill_start_block: startBlock,
    backfill_cursor_block: startBlock,
...
async function readSwapsSince(days = 30) {
